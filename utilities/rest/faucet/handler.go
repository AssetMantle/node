...
func RegisterRESTRoutes(context client.Context, router *mux.Router) {
	handler := func(context client.Context) http.HandlerFunc {
		return func(responseWriter http.ResponseWriter, httpRequest *http.Request) {
			...
			if rest.CheckInternalServerError(responseWriter, queuing.QueueOrBroadcastTransaction(context.WithOutput(responseWriter), base.NewCommonTransactionRequest(faucetKeyName, faucetMemo, context.ChainID, gas, gasAdjustment, 0, 0, 0, sdkTypes.NewCoins(sdkTypes.NewCoin(faucetDenom, sdkTypes.NewInt(faucetFees))), sdkTypes.NewDecCoins(), false), types.NewMsgSend(context.FromAddress, toAddress, sdkTypes.NewCoins(sdkTypes.NewCoin(faucetDenom, sdkTypes.NewInt(faucetAmount)))))) {
				return
			}
