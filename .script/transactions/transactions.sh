...
mantleNode config chain-id test
...
mantleNode keys add $ACCOUNT_NAME_1 $KEYRING
mantleNode keys add $ACCOUNT_NAME_2 $KEYRING
mantleNode keys add $ACCOUNT_NAME_3 $KEYRING
mantleNode keys add $ACCOUNT_NAME_4 $KEYRING

#name users with their addresses
TEST=$(mantleNode keys show -a test $KEYRING)
ACCOUNT_1=$(mantleNode keys show -a $ACCOUNT_NAME_1 $KEYRING)
ACCOUNT_2=$(mantleNode keys show -a $ACCOUNT_NAME_2 $KEYRING)
ACCOUNT_3=$(mantleNode keys show -a $ACCOUNT_NAME_3 $KEYRING)
ACCOUNT_4=$(mantleNode keys show -a $ACCOUNT_NAME_4 $KEYRING)

#Load coins in main
mantleNode tx send $TEST $ACCOUNT_1 10000stake -y $KEYRING $MODE
sleep $SLEEP
#send coins in users
mantleNode tx send $ACCOUNT_1 $ACCOUNT_3 110stake -y $KEYRING $MODE
sleep $SLEEP
mantleNode tx send $ACCOUNT_3 $ACCOUNT_4 10stake -y $KEYRING $MODE
mantleNode tx send $ACCOUNT_1 $ACCOUNT_2 100stake -y $KEYRING $MODE
sleep $SLEEP

#recursively send coins
mantleNode tx send $ACCOUNT_1 $ACCOUNT_3 100stake -y $KEYRING $MODE
mantleNode tx send $ACCOUNT_3 $ACCOUNT_2 50stake -y $KEYRING $MODE
mantleNode tx send $ACCOUNT_2 $ACCOUNT_4 5stake -y $KEYRING $MODE
mantleNode tx send $ACCOUNT_4 $ACCOUNT_2 5stake -y $KEYRING $MODE
sleep $SLEEP
...
mantleNode tx identities nub -y --from $ACCOUNT_1 --nubID $NUB_ID_1 $KEYRING $MODE
mantleNode tx identities nub -y --from $ACCOUNT_2 --nubID $NUB_ID_2 $KEYRING $MODE
mantleNode tx identities nub -y --from $ACCOUNT_3 --nubID $NUB_ID_3 $KEYRING $MODE
sleep $SLEEP
ACCOUNT_1_NUB_ID=$(echo $(mantleNode q identities identities) | awk -v var="$ACCOUNT_1" '{for(i=1;i<=NF;i++)if($i==var)print $(i-6)"|"$(i-3)}')
ACCOUNT_2_NUB_ID=$(echo $(mantleNode q identities identities) | awk -v var="$ACCOUNT_2" '{for(i=1;i<=NF;i++)if($i==var)print $(i-6)"|"$(i-3)}')
ACCOUNT_3_NUB_ID=$(echo $(mantleNode q identities identities) | awk -v var="$ACCOUNT_3" '{for(i=1;i<=NF;i++)if($i==var)print $(i-6)"|"$(i-3)}')
...
mantleNode tx identities define -y --from $ACCOUNT_1 --fromID $ACCOUNT_1_NUB_ID \
...
sleep $SLEEP
IDENTITY_DEFINE_CLASSIFICATION=$(echo $(mantleNode q classifications classifications) | awk -v var="$IDENTITY_DEFINE_IMMUTABLE_META_1_ID" '{for(i=1;i<=NF;i++)if($i==var)print $(i-10)"."$(i-7)}')

mantleNode tx identities issue -y --from $ACCOUNT_1 --fromID $ACCOUNT_1_NUB_ID --classificationID $IDENTITY_DEFINE_CLASSIFICATION --to $ACCOUNT_1 \
...
sleep $SLEEP
IDENTITY_ISSUE_ACCOUNT_1=$(echo $(mantleNode q identities identities) | awk -v var="$IDENTITY_DEFINE_CLASSIFICATION" '{for(i=1;i<=NF;i++)if($i==var)print $i"|"$(i+3)}')
mantleNode tx identities provision -y --from $ACCOUNT_1 --to $ACCOUNT_4 --identityID $IDENTITY_ISSUE_ACCOUNT_1 $KEYRING $MODE
sleep $SLEEP
mantleNode tx identities unprovision -y --from $ACCOUNT_1 --to $ACCOUNT_4 --identityID $IDENTITY_ISSUE_ACCOUNT_1 $KEYRING $MODE
sleep $SLEEP

#metas reveal
mantleNode tx metas reveal -y --from $ACCOUNT_1 --metaFact "S|stringValue$NONCE"
mantleNode tx metas reveal -y --from $ACCOUNT_2 --metaFact "I|identityValue$NONCE"
mantleNode tx metas reveal -y --from $ACCOUNT_3 --metaFact "D|0.101010$NONCE"
mantleNode tx metas reveal -y --from $ACCOUNT_4 --metaFact "H|1$NONCE"
sleep $SLEEP
...
mantleNode tx assets define -y --from $ACCOUNT_1 --fromID $ACCOUNT_1_NUB_ID \
...
mantleNode tx assets define -y --from $ACCOUNT_2 --fromID $ACCOUNT_2_NUB_ID \
...
sleep $SLEEP
ASSET_DEFINE_CLASSIFICATION_1=$(echo $(mantleNode q classifications classifications) | awk -v var="$ASSET_DEFINE_IMMUTABLE_META_1_ID" '{for(i=1;i<=NF;i++)if($i==var)print $(i-10)"."$(i-7)}')
mantleNode tx assets mint -y --from $ACCOUNT_1 --fromID $ACCOUNT_1_NUB_ID --classificationID $ASSET_DEFINE_CLASSIFICATION_1 --toID $ACCOUNT_1_NUB_ID \
...
ASSET_DEFINE_CLASSIFICATION_2=$(echo $(mantleNode q classifications classifications) | awk -v var="$ASSET_DEFINE_IMMUTABLE_META_2_ID" '{for(i=1;i<=NF;i++)if($i==var)print $(i-10)"."$(i-7)}')
mantleNode tx assets mint -y --from $ACCOUNT_2 --fromID $ACCOUNT_2_NUB_ID --classificationID $ASSET_DEFINE_CLASSIFICATION_2 --toID $ACCOUNT_2_NUB_ID \
...
sleep $SLEEP
ASSET_MINT_1=$(echo $(mantleNode q assets assets) | awk -v var="$ASSET_DEFINE_CLASSIFICATION_1" '{for(i=1;i<=NF;i++)if($i==var)print $i"|"$(i+3)}')
ASSET_MINT_2=$(echo $(mantleNode q assets assets) | awk -v var="$ASSET_DEFINE_CLASSIFICATION_2" '{for(i=1;i<=NF;i++)if($i==var)print $i"|"$(i+3)}')

mantleNode tx assets mutate -y --from $ACCOUNT_1 --fromID $ACCOUNT_1_NUB_ID --assetID $ASSET_MINT_1 \
