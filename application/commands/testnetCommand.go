...
func TestnetCommand(moduleBasicManager module.BasicManager, genesisBalancesIterator bankTypes.GenesisBalancesIterator) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "testnet",
...
Example:
	...
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			return InitTestnet(
				clientCtx, cmd, config, moduleBasicManager, genesisBalancesIterator, outputDir, chainID, minGasPrices,
...
func InitTestnet(
	...
	cmd *cobra.Command,
	nodeConfig *tendermintConfig.Config,
	...
	if chainID == "" {
		chainID = "chain-" + tendermintRand.Str(6)
	}

	nodeIDs := make([]string, numValidators)
	valPubKeys := make([]cryptoTypes.PubKey, numValidators)

	simappConfig := serverConfig.DefaultConfig()
	...
	// 	sdkTypes.MsgTypeURL(&ibcchanneltypes.MsgRecvPacket{}),
	// 	sdkTypes.MsgTypeURL(&ibcchanneltypes.MsgAcknowledgement{}),
	// 	sdkTypes.MsgTypeURL(&ibcclienttypes.MsgUpdateClient{}),
	// }
	...
	for i := 0; i < numValidators; i++ {
		...
		accTokens := sdkTypes.TokensFromConsensusPower(1000, sdkTypes.DefaultPowerReduction)
		accStakingTokens := sdkTypes.TokensFromConsensusPower(500, sdkTypes.DefaultPowerReduction)
		coins := sdkTypes.Coins{
			sdkTypes.NewCoin(fmt.Sprintf("%stoken", nodeDirName), accTokens),
			sdkTypes.NewCoin(sdkTypes.DefaultBondDenom, accStakingTokens),
		}

		genBalances = append(genBalances, bankTypes.Balance{Address: addr.String(), Coins: coins.Sort()})
		genAccounts = append(genAccounts, authTypes.NewBaseAccount(addr, nil, 0, 0))

		valTokens := sdkTypes.TokensFromConsensusPower(100, sdkTypes.DefaultPowerReduction)
		createValMsg, err := stakingTypes.NewMsgCreateValidator(
			sdkTypes.ValAddress(addr),
			valPubKeys[i],
			sdkTypes.NewCoin(sdkTypes.DefaultBondDenom, valTokens),
			stakingTypes.NewDescription(nodeDirName, "", "", "", ""),
			stakingTypes.NewCommissionRates(sdkTypes.OneDec(), sdkTypes.OneDec(), sdkTypes.OneDec()),
			sdkTypes.OneInt(),
		)
		if err != nil {
			return err
...
func collectGenFiles(
	clientCtx client.Context, nodeConfig *tendermintConfig.Config, chainID string,
	nodeIDs []string, valPubKeys []cryptoTypes.PubKey, numValidators int,
	outputDir, nodeDirPrefix, nodeDaemonHome string, genBalIterator bankTypes.GenesisBalancesIterator,
) error {
	var appState json.RawMessage
	genTime := tendermintTime.Now()
...
func writeFile(name, dir string, contents []byte) error {
	...
	err := tendermintOS.EnsureDir(dir, 0o755)
	if err != nil {
		return err
	}
